...
dataset generate_dataset_using_evolution_simulation(int seed,
  ...
  sequence_template = (char*)malloc(sizeof(char)*(ds.max_sequence_length+1));
  ...
  for(i=0;i<ds.max_sequence_length;i++) {
    ...
    sequence_template[i] = random_value_i;
    sequence_template[ds.max_sequence_length]= 0;
  }

  for(i=0;i<partition_size;i++) {
    ds.sequences[i] = (char*)malloc(sizeof(char)*(ds.max_sequence_length+1));
    memcpy(ds.sequences[i],sequence_template,
  ...
  while(partition_count < partitions) {
	...
	counter = 0;
	while(random_value_i == sequence_template[mutation_site_i]) {
	  random_value_d=4*((double)rand()/(double)RAND_MAX);
	  random_value_i = (char)random_value_d;
	  if (counter == 100) break;
	}
	sequence_template[mutation_site_i] = random_value_i;
      }
    }
...
int main(int argc, char** argv) {
  ...
  if (argc < 7) {
    printf("Arguments are: \n"
	   ...
	   " constant.\n"
	   " [int] mutation rate\n"
	   " [int] add n mutations to the previous amplification template\n"
	   " [FASTA] output file\n");
    return(1);
  }
